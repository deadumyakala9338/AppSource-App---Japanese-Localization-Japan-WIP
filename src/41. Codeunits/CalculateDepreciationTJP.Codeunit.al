#if AppSource
codeunit 70658578 "Calculate Depreciation TJP"
#endif
#if OnPremise
codeunit 99003 "Calculate Depreciation TJP"
#endif
{
    ...
    var
        ...
        //CalculateNormalDepr: Codeunit "Calculate Normal Depreciation";   //TJP1.00 Add Comment
        CalculateNormalDepr: Codeunit "Calculate Normal Depr. TJP";        //TJP1.00 Add New Code
    ...
    //Parameter FADeprBook change to pFADeprBook due to already having same global variable
    local procedure CheckDeprDaysInFiscalYear(pFADeprBook: Record "FA Depreciation Book"; CheckDeprDays: Boolean; UntilDate: Date)
    var
    ...
    begin
        IsHandled := false;
        OnBeforeCheckDeprDaysInFiscalYear(pFADeprBook, CheckDeprDays, UntilDate, IsHandled);
        if IsHandled then
            exit;

        if DeprBook."Allow more than 360/365 Days" or not CheckDeprDays then
            exit;
        if (pFADeprBook."Depreciation Method" = pFADeprBook."Depreciation Method"::"Declining-Balance 1") or
           (pFADeprBook."Depreciation Method" = pFADeprBook."Depreciation Method"::"DB1/SL")
        then
