#if AppSource
report 70658575 "Gen. FirmBanking Pmt. File TJP"
#endif
#if OnPremise
report 99000 "Gen. FirmBanking Pmt. File TJP"
#endif
{
    ...
    begin
        ...
        repeat
            if (TempGenJournalLine.Amount >= 0) or (TempGenJournalLine.Amount <= 9999999999.0) then begin
                ...
                if not VendorBankAccount.Get(TempGenJournalLine."Account No.", TempGenJournalLine."Vendor Bank Account TJP") then
                    Message(Text001Txt, TempGenJournalLine."Account No.");
                ...
                ItemLine := copystr(InsStr(ItemLine, GetBankNo(BankType::Bank, VendorBankAccount."Zengin Bank Branch No. TJP"), 2), 1, MaxStrLen(ItemLine));

                //Bank Name
                ItemLine := DelStr(ItemLine, 6, 15);
                ItemLine := copystr(InsStr(ItemLine, SetLength(15, IndentType::Left, VendorBankAccount."Zengin Bank Name Kana TJP"), 6), 1, MaxStrLen(ItemLine));

                //Branch Number
                ItemLine := DelStr(ItemLine, 21, 3);
                ItemLine := copystr(InsStr(ItemLine, GetBankNo(BankType::Branch, VendorBankAccount."Zengin Bank Branch No. TJP"), 21), 1, MaxStrLen(ItemLine));

                //Branch Name
                ItemLine := DelStr(ItemLine, 24, 15);
                ItemLine := copystr(InsStr(ItemLine, SetLength(15, IndentType::Left, VendorBankAccount."Zengin Branch Name Kana TJP"), 24), 1, MaxStrLen(ItemLine));

                //Deposit Type
                ItemLine := DelStr(ItemLine, 43, 1);
                IntTemp := VendorBankAccount."Zengin Account Type TJP".AsInteger();
                ItemLine := copystr(InsStr(ItemLine, Format(IntTemp), 43), 1, MaxStrLen(ItemLine));

                //Account Number
                ItemLine := DelStr(ItemLine, 44, 7);
                ItemLine := copystr(InsStr(ItemLine, SetLength(7, IndentType::Right, Format(VendorBankAccount."Zengin Bank Account No. TJP")), 44), 1, MaxStrLen(ItemLine));

                //Recipient Name
                ItemLine := DelStr(ItemLine, 51, StrLen(VendorBankAccount."Zengin Account Holder Kana TJP"));
                ItemLine := copystr(InsStr(ItemLine, VendorBankAccount."Zengin Account Holder Kana TJP", 51), 1, MaxStrLen(ItemLine));
    ...
    begin
        ...
        _GenJournalLine.SetRange("Account Type", _GenJournalLine."Account Type"::Vendor);
        if _GenJournalLine.FindSet() then
            repeat
                _GenJournalLine.TestField("Vendor Bank Account TJP");
    ...
    begin
        case ErrorCheckType of
            0:                //Header
                exit;
            1:                //Data
                if TempGenJournalLine."Vendor Bank Account TJP" = '' then
                    Error(Text008Txt);
            2:                //Trail
                exit;
            3:                //Last
