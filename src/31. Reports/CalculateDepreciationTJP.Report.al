#if AppSource
report 70658588 "Calculate Depreciation TJP"
#endif
#if OnPremise
report 99013 "Calculate Depreciation TJP"
#endif
{
                ...
                if TempFAJnlLine.Find('-') then begin
                    ...
                    if DocumentNo = '' then
                        if FAJnlLine.FindLast() then
                            DocumentNo2 := FAJnlLine."Document No."
                        else
                ...
                if TempGenJnlLine.Find('-') then begin
                    ...
                    if DocumentNo = '' then
                        if GenJnlLine.FindLast() then
                            DocumentNo2 := GenJnlLine."Document No."
                        else
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ReqUseForceNoOfDays; UseForceNoOfDays)
                    {
                        ApplicationArea = FixedAssets;
                    ...
                    field(ReqPostingDate; PostingDate)
                    {
                        ApplicationArea = FixedAssets;
                    ...
                    field(ReqDocumentNo; DocumentNo)
                    {
                        ApplicationArea = FixedAssets;
                        Caption = 'Document No.';
                    ...
                    field(ReqPostingDescription; PostingDescription)
                    {
                        ApplicationArea = FixedAssets;
    ...
    begin
        ...
        if DeprUntilDate = 0D then
            Error(Text000Msg, FAJnlLine.FieldCaption("FA Posting Date"));
    ...
    var
        ...
        //CalculateDepr: Codeunit "Calculate Depreciation";    //TJP1.00 Comment
        CalculateDepr: Codeunit "Calculate Depreciation TJP";  //TJP1.00 Add New Code
        ...
        CompletionStatsFAJnlQst: Label 'The depreciation has been calculated.\\%1 fixed asset journal lines were created.\\Do you want to open the Fixed Asset Journal window?', Comment = '%1= FA Journal, The depreciation has been calculated.\\5 fixed asset journal lines were created.\\Do you want to open the Fixed Asset Journal window?';
        CompletionStatsGenJnlQst: Label 'The depreciation has been calculated.\\%1 fixed asset G/L journal lines were created.\\Do you want to open the Fixed Asset G/L Journal window?', Comment = '%1= FA Journal, The depreciation has been calculated.\\2 fixed asset G/L  journal lines were created.\\Do you want to open the Fixed Asset G/L Journal window?';
        DeprUntilDateModified: Boolean;
        Text000Msg: Label 'You must specify %1.', Comment = '%1=Posting Date';
        Text001Msg: Label 'Force No. of Days must be activated.';
        Text002Msg: Label '%1 and %2 must be identical. %3 must be %4 in %5 %6 = %7.', Comment = '%1=FA Posting Date,%2=Posting Date,%3=GL Posting Date,%4=No,%5=FA,%6=FA,%7=FA Code';
        Text003Msg: Label 'Depreciating fixed asset      #1##########\', Comment = '%1=FA Count';
        Text004Msg: Label 'Not depreciating fixed asset  #2##########\', Comment = '%1=FA Count';
        Text005Msg: Label 'Inserting journal lines       #3##########', Comment = '%1=JV Count';
        Text006Msg: Label 'Use Force No. of Days must be activated.';
